from numpy import random, exp, max, dot, sum, array, append, argmax, round, average
...
class NeuralNetwork:
    ...
    def softmax(self, x, deriv=False):
        if deriv:
            return x * (1 - x)
        exp_x = exp(x - max(x))
        return exp_x / sum(exp_x, axis=0)

    def forwardprop(self, x):
        self.hidden_layer = self.sigmoid(dot(self.weights_1, x) + self.bias1)
        self.output_layer = self.softmax(dot(self.weights_2, self.hidden_layer) + self.bias2)

        return self.output_layer

    def backwardprop(self, x, ideal):
        output_layer_error = self.output_layer - ideal
        output_layer_delta = output_layer_error * self.softmax(self.output_layer, deriv=True)
    ...
    def predict(self, x, y=False):
        ...
        return argmax(self.forwardprop(x)), argmax(y)
